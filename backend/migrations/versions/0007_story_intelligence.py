...
def upgrade() -> None:
    ...
    # ── story_intel_jobs ──────────────────────────────────────────────────────
    if not _table_exists(bind, "story_intel_jobs"):
        op.create_table(
            "story_intel_jobs",
    ...
    if not _index_exists(bind, "story_intel_jobs", "ix_story_intel_jobs_story_id"):
        op.create_index("ix_story_intel_jobs_story_id", "story_intel_jobs", ["story_id"])

    # ── story_genre_hierarchy ─────────────────────────────────────────────────
    if not _table_exists(bind, "story_genre_hierarchy"):
        op.create_table(
            "story_genre_hierarchy",
    ...
    # ── story_dna ─────────────────────────────────────────────────────────────
    if not _table_exists(bind, "story_dna"):
        op.create_table(
            "story_dna",
    ...
    if not _table_exists(bind, "story_audience_profile"):
        op.create_table(
            "story_audience_profile",
    ...
    # ── story_themes ──────────────────────────────────────────────────────────
    if not _table_exists(bind, "story_themes"):
        op.create_table(
            "story_themes",
    ...
    # ── story_conflicts ───────────────────────────────────────────────────────
    if not _table_exists(bind, "story_conflicts"):
        op.create_table(
            "story_conflicts",
    ...
    # ── story_world_profile ───────────────────────────────────────────────────
    if not _table_exists(bind, "story_world_profile"):
        op.create_table(
            "story_world_profile",
    ...
    if not _table_exists(bind, "story_narrative_structure"):
        op.create_table(
            "story_narrative_structure",
    ...
    # ── story_emotional_arc ───────────────────────────────────────────────────
    if not _table_exists(bind, "story_emotional_arc"):
        op.create_table(
            "story_emotional_arc",
    ...
    # ── story_pacing_map ──────────────────────────────────────────────────────
    if not _table_exists(bind, "story_pacing_map"):
        op.create_table(
            "story_pacing_map",
    ...
    if not _table_exists(bind, "story_strength_indicators"):
        op.create_table(
            "story_strength_indicators",
    ...
    # ── story_risk_register ───────────────────────────────────────────────────
    if not _table_exists(bind, "story_risk_register"):
        op.create_table(
            "story_risk_register",
    ...
    if not _table_exists(bind, "story_foreshadowing_registry"):
        op.create_table(
            "story_foreshadowing_registry",
    ...
    if not _table_exists(bind, "story_continuity_risks"):
        op.create_table(
            "story_continuity_risks",
    ...
    if not _table_exists(bind, "story_intel_versions"):
        op.create_table(
            "story_intel_versions",
    ...
    if not _index_exists(bind, "story_intel_versions", "ix_story_intel_versions_story_id"):
        op.create_index("ix_story_intel_versions_story_id", "story_intel_versions", ["story_id"])

    # ── character_intelligence ────────────────────────────────────────────────
    if not _table_exists(bind, "character_intelligence"):
        op.create_table(
            "character_intelligence",
    ...
    if not _index_exists(bind, "character_intelligence", "ix_char_intel_story_id"):
        op.create_index("ix_char_intel_story_id", "character_intelligence", ["story_id"])

    # ── relationship_intelligence ─────────────────────────────────────────────
    if not _table_exists(bind, "relationship_intelligence"):
        op.create_table(
            "relationship_intelligence",
            sa.Column("intel_id",             sa.String(), primary_key=True),
    ...
    if not _index_exists(bind, "relationship_intelligence", "ix_rel_intel_story_id"):
        op.create_index("ix_rel_intel_story_id", "relationship_intelligence", ["story_id"])

    # ── story_memory_entries ──────────────────────────────────────────────────
    if not _table_exists(bind, "story_memory_entries"):
        try:
    ...
    if not _index_exists(bind, "story_memory_entries", "ix_memory_story_id"):
        op.create_index("ix_memory_story_id",   "story_memory_entries", ["story_id"])
    if not _index_exists(bind, "story_memory_entries", "ix_memory_type"):
        op.create_index("ix_memory_type",       "story_memory_entries", ["memory_type"])
    if not _index_exists(bind, "story_memory_entries", "ix_memory_entity_id"):
        op.create_index("ix_memory_entity_id",  "story_memory_entries", ["entity_id"])

    # ── story_timeline ────────────────────────────────────────────────────────
    if not _table_exists(bind, "story_timeline"):
        op.create_table(
            "story_timeline",
    ...
    if not _table_exists(bind, "story_timeline_events"):
        op.create_table(
            "story_timeline_events",
    ...
    if not _index_exists(bind, "story_timeline_events", "ix_timeline_events_story_id"):
        op.create_index("ix_timeline_events_story_id",   "story_timeline_events", ["story_id"])
    if not _index_exists(bind, "story_timeline_events", "ix_timeline_events_chapter_no"):
        op.create_index("ix_timeline_events_chapter_no", "story_timeline_events", ["chapter_number"])

    # ── story_graph_nodes ─────────────────────────────────────────────────────
    if not _table_exists(bind, "story_graph_nodes"):
        try:
    ...
    if not _index_exists(bind, "story_graph_nodes", "ix_graph_nodes_story_id"):
        op.create_index("ix_graph_nodes_story_id",  "story_graph_nodes", ["story_id"])
    if not _index_exists(bind, "story_graph_nodes", "ix_graph_nodes_node_type"):
        op.create_index("ix_graph_nodes_node_type", "story_graph_nodes", ["node_type"])
    if not _index_exists(bind, "story_graph_nodes", "ix_graph_nodes_entity_id"):
        op.create_index("ix_graph_nodes_entity_id", "story_graph_nodes", ["entity_id"])

    # ── story_graph_edges ─────────────────────────────────────────────────────
    if not _table_exists(bind, "story_graph_edges"):
        op.create_table(
            "story_graph_edges",
    ...
    if not _index_exists(bind, "story_graph_edges", "ix_graph_edges_story_id"):
        op.create_index("ix_graph_edges_story_id",  "story_graph_edges", ["story_id"])
    if not _index_exists(bind, "story_graph_edges", "ix_graph_edges_from_node"):
        op.create_index("ix_graph_edges_from_node", "story_graph_edges", ["from_node_id"])
    if not _index_exists(bind, "story_graph_edges", "ix_graph_edges_to_node"):
