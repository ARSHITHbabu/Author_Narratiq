...
>
> ⚑ Superseded — the Stage 2 note below is kept for traceability.
>
> ### ⚑ 2026-09-25 — Stage 2 cloud pass: implementation complete, gate OPEN — manual verification pending
>
> Executed under the two-agent Cloud workflow (Implementation Agent + Independent Technical Review Agent), on review branch `claude/project-thread-yeshmd`. Task 2.5 closed; 2.2 and 2.6 remain open only on pod/author checks listed in `docs/testing/manual-verification/stage-02-manual-verification-guide.md` (MV-2.2-A, MV-2.2-B, MV-2.5-A, MV-2.6-A, MV-2.6-B). **Next: Stage 5**, the next stage in order with open items (fresh analysis), after the Stage 2 completion review. Per the author's instruction in the project chat on 2026-09-25 at 09:55 UTC (the Main Project Coordinator workflow message, which directs work through **all** remaining stages), the earlier "do not plan Stage 8 without instruction" hold is lifted for this run.
>
> ⚑ Superseded — the Stage 7 note below is kept for traceability.
>
...
- [x] **2.1 — Audit and clean RunPod UI environment variables** — *2026-09-21, verified live: direct inspection of the pod's OS environment (which is where RunPod UI variables land) found none of the app-relevant or obsolete variables set — no `VLLM_BASE_URL`, `DATABASE_URL`, `CORS_ORIGINS`, `MODEL_BASE_DIR`, `HF_TOKEN`, `NEXT_PUBLIC_API_URL`, or any §7 obsolete key. Only `SECRET_KEY` is set (see 2.2) — ⚠ *disputed 2026-09-25: `runpod-environment-variables.md` §11 item 1, written the same day, says `SECRET_KEY` was **not** set; that pod has since been reset, so the current state is routed to MV-2.2-A in `docs/testing/manual-verification/stage-02-manual-verification-guide.md`.* Backend started with no pydantic-settings error and `/api/health` reports `"vllm": "ready"`.*
...
- [ ] **2.2 — Verify `SECRET_KEY` presence and stability** — *2026-09-21: stability fully verified; the team-secret-store subtask is a human/organisational action outside what this session can perform or confirm, so the main box stays open.*
  ...
  - **Implementation checklist:**
    ...
    - [ ] Confirm it is set in the RunPod UI, not only auto-generated into `backend/.env`, so a re-clone does not invalidate sessions — ⚠ **Unticked 2026-09-25 as DISPUTED** (Stage 2 plan review, finding M2): the evidence below conflicts with `runpod-environment-variables.md` §11 item 1, recorded the same day, which says no `SECRET_KEY` was in the OS environment. Both describe a pod that has since been reset (current: `shbr4txyem5s9w`), so neither can be re-checked from here. Re-tick only on the evidence of **MV-2.2-A** (`docs/testing/manual-verification/stage-02-manual-verification-guide.md`). *Original note: 2026-09-21: confirmed present in the pod's OS environment (i.e. the RunPod UI), and SHA-256 of the `.env` file's `SECRET_KEY=` line matched before and after this session's pod restart — value never displayed in plaintext during that comparison*
    - [ ] Record it in the team secret store — *not performed; no team secret store is reachable or nameable from this session — needs a human/ops action outside this environment. 2026-09-25: a safe storage procedure (clipboard-only copy, SHA-256 comparison, restore and rotation steps) is now in `runpod-environment-variables.md` §3.3; the action itself is **MV-2.2-B** in `docs/testing/manual-verification/stage-02-manual-verification-guide.md`.*
  - **Verification:**
...
- [x] **2.5 — Close the UNVERIFIED items in the environment document** — *2026-09-25: CLOSED. §11 item 2's override case is now demonstrated by four production builds on the locked `next@14.2.35` (OS `NEXT_PUBLIC_API_URL` beats `.env.local`: OSENV sentinel 1 / ENVLOCAL 0), so §11 contains no untested assumption. Two §11 consistency errors were corrected (the intro named item 4 as open; item 1's `SECRET_KEY` statement conflicts with task 2.2 and is now marked disputed → MV-2.2-A). Supporting fix: `start-narratiq.sh:622` now passes `NEXT_PUBLIC_API_URL` explicitly to its build (verified in scratch build 4); real-pod confirmation is MV-2.5-A. Previous note: 2026-09-21: 6 of 7 §11 items are Confirmed with evidence; item 2 (Next.js env-precedence override case) explicitly remains a demonstrated-default-only / inferred-override finding, so one genuine untested assumption remains and the Definition of Done is not literally met. No task 11.5 exists yet in the current Stage 11 to route it to.*
  ...
  - **Implementation checklist:**
    ...
    - [x] Feed remaining unknowns into task 11.5 — *2026-09-25: no unknowns remain to feed. The "11.5" reference is **stale**: today's 11.5 is "Refresh the product and technical specification to v5", unrelated to environment unknowns; the reference is left in place, annotated, and not renumbered. §11 item 4 is not an unknown but an open design question (should `start-narratiq.sh` honour a pre-set `MODEL_BASE_DIR`?), which stays tracked in `runpod-environment-variables.md` §10 follow-ups. Ticking approved by the Stage 2 plan review. Previous note: not done — Stage 11 currently has no task numbered 11.5 to receive it*
  - **Verification:**
    - [x] Every §11 item has a recorded outcome — *true regardless of how favourable each outcome is; all 7 rows are filled in*
  - **Definition of done:** The environment document contains no untested assumption. ✅ *(2026-09-25 — met: the override case is demonstrated; see `runpod-environment-variables.md` §11 item 2 for the builds and counts. Previously not met.)*
...
- [ ] **2.6 — Establish a repeatable clean-start verification script** — *2026-09-21: 3 of 4 implementation subtasks are in `scripts/verify_runpod_setup.sh` and confirmed working; the RunPod-port-list check was never added, and the failure-mode half of verification was not exercised.*
  ...
  - **Implementation checklist:**
    ...
    - [x] Add a check that ports 3000/8000 appear in the RunPod port list — *2026-09-25: implemented in `scripts/verify_runpod_setup.sh` ("RunPod Port Exposure"). Reads `pod.runtime.ports` from the RunPod GraphQL API (the query shape used live in the port-3000 incident) with the pod-injected `RUNPOD_API_KEY` sent as a header on stdin, never in a URL/argv/output; exact `privatePort` + `type=http` matching; FAIL only when the API answers and a port is missing, WARN when it cannot be determined. Covered by hermetic tests N3–N6, L1, S1 (`scripts/tests/test_verify_runpod_setup.py`, 23/23). **The live response shape, Bearer-header acceptance and key permission are unverified** → MV-2.6-A in `docs/testing/manual-verification/stage-02-manual-verification-guide.md`. Previous note: not implemented — `grep` for `RUNPOD_API_KEY`/`api.runpod` in the script found nothing; `RUNPOD_API_KEY` is available in the pod environment but the script never calls the RunPod API*
  - **Verification:**
    - [ ] Script passes on the current pod and fails correctly on a deliberately broken config — *first half confirmed (36/0, exit 0, 2026-09-21); second half not exercised this session — no config was deliberately broken to test the negative case. 2026-09-25: a strict `--expect-running` mode now makes a stopped service or proxy 502 a FAIL (before, a crashed service still gave "All checks passed"), and the failure paths are proven against local stubs (strict/default modes, stale backend vLLM status, vLLM `/health` 500, dead vLLM port, missing/mismatched RunPod ports). Three further false-pass defects were found and fixed: no `ss`/`netstat` → every running service reported NOT STARTED; `curl -s` treated a vLLM `/health` 500 as healthy; an unreachable proxy (curl `000`) was only a warning. Still open because the current pod was not available: **MV-2.6-B** in `docs/testing/manual-verification/stage-02-manual-verification-guide.md`*
  - **Definition of done:** Environment health is one command, not a manual ritual. *(⚑ Superseded 2026-09-21 note, kept for traceability: "Close, but not complete — the port-list check is missing and the negative case is unproven.")* *(Current, 2026-09-25: the port-list check exists and the negative case is proven in hermetic tests; completion awaits the real-pod runs MV-2.6-A and MV-2.6-B.)*
...
- [ ] `SECRET_KEY` stable and stored — *stability fully proven 2026-09-21 (byte-identical across a real restart, pre-restart JWTs still validate); "stored" only as far as the RunPod UI — the team-secret-store copy remains undone, see 2.2. ⚠ 2026-09-25: even the RunPod-UI part is now disputed (see 2.2 subtask 2); pending MV-2.2-A and MV-2.2-B*
...
- [x] Environment document UNVERIFIED items closed — *2026-09-25: 7 of 7 — §11 item 2's override case demonstrated by build (see task 2.5). Previously 6 of 7*
- [ ] Clean-start verification is a single repeatable command — *script exists and passes, but is missing the RunPod port-list check (2.6) and its failure path is unproven. 2026-09-25: port-list check added and `bash scripts/verify_runpod_setup.sh --expect-running` is the one command; failure paths proven in hermetic tests; real-pod proof pending MV-2.6-A/B*
...
> **2026-09-25 — Stage 2 cloud implementation pass (two-agent workflow; plan approved with changes by the Independent Technical Review Agent).** Repository implementation is complete; the gate stays **OPEN — MANUAL VERIFICATION PENDING**. **2.5 closed** (override case demonstrated by build; §11 corrected). **2.6:** port-list subtask ticked; the verification box and parent stay open on MV-2.6-A/B. **2.2:** subtask 2 **unticked as disputed** (conflicting same-day records, pod since reset → MV-2.2-A); the secret-store subtask stays open with a safe procedure documented (MV-2.2-B). Net checkbox change in this section: +4 ticked, −1 unticked (net +3; 35 → 38 ticked of the section's boxes). Every remaining item has exact steps in `docs/testing/manual-verification/stage-02-manual-verification-guide.md`. Stage 2 cannot close without the pod and the author; later stages do not depend on it.
...
- [ ] **5.14 — Story audit: continuity false positives and reasoning depth** *(2026-09-21 Stage 5 closure status: citation-suppression core + 8 of 11 deeper items done and measured; **3 items genuinely PARTIAL and left unticked** — timeline reasoning, narrative reasoning, relationship arc section. This task stays open; see below)*
  ...
  - **Blocked by:** None
  - **Can run in parallel:** Yes
  - **Context:** False continuity breaks are worse than none — they train authors to ignore the feature. *2026-09-21 update — the architectural landscape turned out to be different from what the first implementation pass assumed:* there are actually **three separate story-analysis surfaces** in this codebase, not one — `check_continuity` (continuity-check), `analyze_manuscript` (the separate, already-built `/manuscript-report` editorial report, which already had `character_arcs`, `pacing`, `unresolved_threads`, `strengths`, `improvements` with a "developmental editor" framing and chapter-citation requirements), and `narrative_threads.py`'s own fully-built deterministic thread-lifecycle scanner. This round's work wires these together and deepens `check_continuity` itself, rather than building 11 new subsystems from scratch — see each item below for exactly what changed and what's genuinely still open. *2026-09-25 correction (Stage 5 cloud pass, plan review H2):* "three surfaces" was incomplete. The **Story Intelligence** passes are a fourth, with LLM-derived timeline events (P26, `story_timeline_events`), a foreshadowing registry (P11) and relationship intelligence (P24). Decision: the ChapterSummary-based modules stay the source. Story Intelligence is consumed only from **non-stale** rows and only where it adds something: P26 `is_flashback`/`is_flashforward` suppresses timeline candidates, and P11 `orphaned_hints` add setup-without-payoff candidates. P24 is deliberately not consumed, so there is one author-facing relationship view, not two that can disagree (`services/signal_inputs.py`).
  - **Implementation checklist:**
    ...
    - [ ] Strengthen timeline reasoning (Critical 4) — **PARTIAL, unticked at Stage 5 closure (2026-09-21)** (was previously ticked while labelled partial — corrected, since a ticked box counts as complete). The "timeline" contradiction type and the general cause-and-effect instruction above both apply to timeline claims, but no DEDICATED timeline-ordering logic (e.g. explicit date/sequence checking) was added, and no isolated measurement of timeline-specific recall was run — the improvement here is real but shared with items 1/3/5, not independently verified for this dimension alone. *2026-09-25 (Stage 5 cloud pass): a dedicated mechanism now exists — IMPLEMENTED, MANUAL VERIFICATION REQUIRED; stays unticked pending live measurement and the author's acceptance (MV-5.14-A, MV-5.14-D).* `services/timeline_signals.py` parses `timeline_markers` and `key_events` into anchors (full dates, bare years, weekdays, seasons, stated ages, flashback cues) and emits cited candidates (date reversal, age decrease, weekday/date mismatch, season reversal), suppressed by flashback cues and by non-stale P26 flashback/flash-forward chapters, capped at 8. They are fed to `check_continuity` as a "verify each" block (prompt **v3**, continuity now in the 5.1 registry); every finding still passes `validate_continuity_citations`. Cloud: `tests/test_stage5_signals.py` (planted reversal 100% recall, flashback control 0 candidates) and `tests/test_stage5_persistence.py::test_continuity_check_passes_timeline_and_flash_suppressed_signals`
    - [ ] Strengthen narrative reasoning (Critical 5) — **PARTIAL, unticked at Stage 5 closure (2026-09-21)**, same basis as timeline reasoning above: the shared cause-and-effect instruction applies, but no dedicated narrative-logic mechanism was built beyond it. *2026-09-25 (Stage 5 cloud pass): IMPLEMENTED, MANUAL VERIFICATION REQUIRED; stays unticked pending the author's acceptance (MV-5.14-B, MV-5.14-D).* `services/narrative_signals.py` flags setups without payoff (dead-end threads, plus non-stale P11 orphaned hints that cite a real chapter), characters who disappear (the scanner's max(3, 15%) rule) and runs of chapters with no recorded purpose. The signals go to `check_continuity` (verify-each, citation-validated) and into `/manuscript-report` as a labelled "Things to check" section. Cloud: `tests/test_stage5_signals.py`
    ...
    - [ ] Add relationship arc analysis (Medium 11) — **PARTIAL, unticked at Stage 5 closure (2026-09-21)**. `ChapterSummary.relationship_changes` is now wired into `check_continuity`'s prompt AND its Tier-2 groundedness check (a relationship-based contradiction can now be both detected and citation-verified — confirmed live, see the report), and `/manuscript-report`'s `character_arcs` covers character-level arcs. There is no DEDICATED "relationship arc across the manuscript" section distinct from these two — a real but partial advance, not the full longitudinal relationship-arc view the item implies. *2026-09-25 (Stage 5 cloud pass): IMPLEMENTED, MANUAL VERIFICATION REQUIRED; stays unticked pending the author's acceptance (MV-5.14-C, MV-5.14-D).* `services/relationship_arcs.py` aggregates `relationship_changes` by unordered character pair in chapter order (ids resolved to current names; deleted ids dropped and counted), returned as `relationship_arcs` and shown as a "Relationships" section with chapter chips in the Manuscript Report panel. Deterministic, no LLM call. Cloud: `tests/test_stage5_signals.py`, `tests/test_stage5_persistence.py::test_report_round_trip_post_then_get`
...
>
> **5.14's 3 partial items (dedicated timeline-reasoning logic, dedicated narrative-reasoning logic, relationship arc as its own section) — acceptance decision explicitly DEFERRED by the author, not accepted, not rejected.** *(2026-09-25: dedicated mechanisms for all three are now implemented — see 5.14. The decision is still the author's: MV-5.14-D.)* Status stays exactly as already recorded at 5.14 (partial, unticked) until a final call is made.
>
...
#### Stage 5 live-review defects (found 2026-09-22, fixed in the 2026-09-25 cloud pass)
...
- [x] **8.2 — Resizable and expandable panels** — *2026-09-25: cloud*
  ...
  - **Can run in parallel:** Yes
  - **Implementation checklist:**
    - [x] Make the side panel resizable by drag — *2026-09-25: binder and sidecar drag handles; `tests/studio/layout.spec.ts`*
    - [x] Allow panels to expand to full width for detailed work — *2026-09-25: **deviation** — the AI sidecar expands to 65 % with the binder hidden (Ctrl+\\); Analyze tools do open full width. Recorded as a deviation in the Stage 8 report*
...
- [ ] **8.4 — Progressive disclosure** — *2026-09-25: implemented; open on author review*
  ...
  - **Implementation checklist:**
    ...
    - [x] Establish a clear tool hierarchy — *2026-09-25: workspace → section → tool, from `lib/registries/toolHomes.ts`; sidecar groups in `AIToolsSidebar`*
    - [x] Reduce navigation density — *2026-09-25: Write controls counted in `tests/studio/modes.spec.ts` (53→35 open)*
...
- [ ] **8.5 — Drafting and editing mode separation** — *2026-09-25: implemented; open on author review*
  ...
  - **Implementation checklist:**
    ...
    - [x] Respect the long-form writing workflow — *2026-09-25: per-story Draft/Edit mode and Reading mode, `tests/studio/modes.spec.ts`; author judgement in MV-8.5*
  - **Verification:**
...
- [x] **8.8 — Information architecture and navigation deduplication** — *2026-09-25: cloud*
  ...
  - **Implementation checklist:**
    ...
    - [x] Rework the IA so it reads as an author studio, not a tool dashboard — *2026-09-25: 7 workspaces with sections, `lib/registries/toolHomes.ts`; author judgement in MV-8.5/MV-8.7*
    - [x] Give every feature one clearly defined location — *2026-09-25: one row per tool in `toolHomes.ts`, enforced by `tests/tool-homes.spec.ts`*
  - **Verification:**
...
- [ ] **8.9 — Accessibility baseline** — *2026-09-25: implemented; open on screen-reader pass and CI*
  ...
  - **Can run in parallel:** Yes
  - **Implementation checklist:**
    - [x] Adopt WCAG 2.1 AA as the standard — *2026-09-25: axe run with the WCAG 2.0 and 2.1 A/AA tags in `tests/studio/a11y.spec.ts`*
...
- [x] **8.10 — Responsive behaviour** — *2026-09-25: cloud*
  ...
  - **Can run in parallel:** Yes
  - **Implementation checklist:**
    - [x] Verify the workspace at common laptop resolutions — *2026-09-25: `tests/studio/responsive.spec.ts` (1920, 1536, 1440, 1366, 1280)*
    - [x] Verify panels degrade gracefully at narrow widths — *2026-09-25: `tests/studio/responsive.spec.ts` (1024, 768)*
    - [x] Confirm no horizontal page scroll — *2026-09-25: `tests/studio/responsive.spec.ts`, every workspace*
  - **Verification:**
...
- [x] Phase 2 Issue 10 closed — *2026-09-25: task 8.8; triage register P2-10*
- [x] Every Phase 1, 2 and 3 tool has one clear home — *2026-09-25: `tests/tool-homes.spec.ts`*
- [x] Panels resizable; layout persists — *2026-09-25: `tests/studio/layout.spec.ts`*
- [ ] Accessibility baseline met — *open — screen-reader pass and CI check*
- [ ] Multi-hour author usability session passed — *open — 8.11*
- [x] No feature duplicated across navigation sections — *2026-09-25: `tests/tool-homes.spec.ts`; the one documented second view is the Write binder's read-only idea markers (D10)*
